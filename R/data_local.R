...
#' Prepare junction quantification files to be loaded into psichomics
#' 
#' @param ... File(s) to read
#' @param samples Character: sample names
#' @param output Character: path of output file (if NULL, only returns the data
#' without save to a file)
#' @param startOffset Numeric: value to offset start position
#' @param endOffset Numeric: value to offset end position
#' 
#' @importFrom data.table fwrite
#' 
#' @return Prepared junction quantification file
#' @export
#' 
#' @examples
#' \dontrun{
#' prepareJunctionQuant(junctionFile1, junctionFile2, junctionFile3, 
#'                      junctionFile4, samples=c("Control rep1", "Control rep2",
#'                                               "KD rep1", "KD rep2"))
#' }
prepareJunctionQuant <- function(..., samples=NULL, 
                                 output="psichomics_junctions.txt",
                                 startOffset=NULL, endOffset=NULL) {
    # Parse files to identify software used to quantify splice junctions
    ...
    if (length(samples) != length(files)) {
        if (!is.null(samples)) {
            warning("Number of sample names is larger than available samples. ",
                    "Default sample names were used instead.")
        }
        samples <- paste("sample", seq(files), sep="_")
    }
    ...
    if (!is.null(output)) {
        ...
        fwrite(junctionQuant, output, sep="\t", na=0)
    }
    return(junctionQuant)
}

#' @rdname prepareJunctionQuant
...
prepareJunctionQuantSTAR <- function(..., startOffset=-1, endOffset=+1) {
    ...
    # duplicate entry and append both a positive and negative sign
